...
-- Repeat the last f/F/t/T (flash char) motion on <C-n> / <C-p> instead of the
-- default ; / , -- n = forward (next), p = back (prev). remap=true routes them
-- through flash's own ; / , handlers (so the labels still show). ; and , keep
-- working too. (<C-l> is left for Harpoon slot 3.) In foldfloat popups
-- <C-n>/<C-p> are remapped buffer-local (entry nav), which take precedence there.
vim.keymap.set({ "n", "x", "o" }, "<C-n>", ";", { remap = true, desc = "Flash repeat (forward)" })
vim.keymap.set({ "n", "x", "o" }, "<C-p>", ",", { remap = true, desc = "Flash repeat (back)" })
